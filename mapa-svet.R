...
write.csv("pocetpartnerstvi.csv", pocetPartnerstvi)
...
for (i in 1:nrow(vztahy)) {
  ...
  if (vztahy[i,4]=="Brno") {
    lines(spojnice, col=paleta[1], lwd=0.1)
  } 
  else if (vztahy[i,4]=="Praha")
  {lines(spojnice, col=paleta[2], lwd=0.1)
  }
  else if (vztahy[i,4]=="Ostrava")
  {lines(spojnice, col=paleta[3], lwd=0.1)
  }
  else if (vztahy[i,4]=="Žamberk")
  {lines(spojnice, col=paleta[4], lwd=0.1)
  }
  else if (vztahy[i,4]=="Znojmo")
  {lines(spojnice, col=paleta[5], lwd=0.1)
  }
  else if (vztahy[i,4]=="Kutná Hora")
  {lines(spojnice, col=paleta[6], lwd=0.1)
  }
  else if (vztahy[i,4]=="Hradec Králové")
  {lines(spojnice, col=paleta[7], lwd=0.1)
  }
  else if (vztahy[i,4]=="Bystré")
  {lines(spojnice, col=paleta[8], lwd=0.1)
  }
  
  else {
  lines(spojnice, lwd=0.03)
  }
}
